...
class Agent
{
  ...
  public:
	...
	action_t search(int num_search, const void *backdoor_to_real_env=NULL);
	...
	void genPercept(percept_t &percept, percept_t &r) const;
	...
	void genPerceptAndUpdate(percept_t &percept, percept_t &r);
	...
	bool getLastUpdate(void) const {return m_last_update_percept;};

	void pecodePercept(const symbol_list_t &symlist, percept_t &observation, percept_t &reward);
  private:
	// action sanity check
	...
	percept_t decodePercept(const symbol_list_t &symlist) const;
